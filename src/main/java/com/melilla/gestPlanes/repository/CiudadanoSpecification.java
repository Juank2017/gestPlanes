...
private PlanService planService;
	...
	public Predicate toPredicate(Root<Ciudadano> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		...
		if (this.criteria.getId().equals("fechaRegistro") || 
			this.criteria.getId().equals("fechaInicio")||
			this.criteria.getId().equals("fechaFin")||
			this.criteria.getId().equals("fechaExtincion")
			) {
		...
		if (likePredicate != null) predicados.add(likePredicate);
		if (fechaPredicate != null) predicados.add(fechaPredicate);
