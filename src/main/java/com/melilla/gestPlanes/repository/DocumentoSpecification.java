...
public class DocumentoSpecification implements Specification<Documento> {
	...
	public DocumentoSpecification(DocumentoCriterioBusqueda criterios, PlanService plan) {
	...
	public Predicate toPredicate(Root<Documento> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		...
		if (this.criterios.getId().equals("DNI")) {
			likePredicate = builder.like(documentoCiudadano.<String>get(criterios.getId()),
					"%" + criterios.getValues() + "%");
