...
public class ParteBajaServiceIMPL implements ParteBajaService {
	...
	public List<ParteBaja> obtenerPartesBajaTrabajador(long idTrabajador) {
		...
		return parteBajaRepository.findAllByCiudadanoIdCiudadanoAndDeletedFalse(idTrabajador);
	}

	@Override
	public List<Map<String, String>> obtenerPartesBajaTrabajadorMap(long idTrabajador) {

		List<ParteBaja> partes = parteBajaRepository.findAllByCiudadanoIdCiudadanoAndDeletedFalse(idTrabajador);
	...
	private List<Map<String, String>> listaParteBajaToListaParteBajaPlana(List<ParteBaja> partes) {
		...
		while (it.hasNext()) {
			...
			if (!parte.isDeleted()) {
				...
				partePlano.put("fechaInicioBaja",
						(parte.getFechaInicioBaja() != null) ? parte.getFechaInicioBaja().format(dtf) : "");
				partePlano.put("fechaFinBaja",
						(parte.getFechaFinBaja() != null) ? parte.getFechaFinBaja().format(dtf) : "");
				partePlano.put("contingencia", parte.getContingencia().getIdTipoContingencia() + " - "
						+ parte.getContingencia().getContingencia());
	...
	public ParteBaja altaParteBaja(CrearParteBajaDTO parte) {
		...
		Ciudadano trabajador = ciudadanoRepository.findById(parte.getIdCiudadano())
				.orElseThrow(() -> new CiudadanoNotFoundException(parte.getIdCiudadano()));

		nuevoParte.setContingencia(tipoContingenciaRepository.findById(parte.getIdTipoContingencia())
	...
	public ParteBaja editaParteBaja(EditaParteBajaDTO parte) {
		...
		ParteBaja parteAntiguo = parteBajaRepository.findById(parte.getIdParteBaja())
				.orElseThrow(() -> new GenericNotFoundException());

		parteAntiguo.setContingencia(tipoContingenciaRepository.findById(parte.getIdTipoContingencia())
				.orElseThrow(() -> new GenericNotFoundException()));
		...
		parteAntiguo.getCiudadano()
				.setBajaLaboral(ciudadanoService.estaDeBaja(parteAntiguo.getCiudadano().getIdCiudadano()));
	...
	public ParteBaja insertaParteConfirmacion(CrearParteConfirmacionDTO parte) {
		...
		ParteBaja parteBaja = parteBajaRepository.findById(parte.getIdParteBaja())
				.orElseThrow(() -> new GenericNotFoundException());
	...
	public TipoContingencia editarContingencia(EditaContingenciaDTO contingencia) {
		...
		TipoContingencia tipo = tipoContingenciaRepository.findById(contingencia.getIdContingencia())
				.orElseThrow(() -> new GenericNotFoundException());
	...
	public void borraContingencia(long idTipoContingencia) {
		...
		TipoContingencia tipo = tipoContingenciaRepository.findById(idTipoContingencia)
				.orElseThrow(() -> new GenericNotFoundException());
	...
	public List<Map<String, String>> obtenerPartesBajaPlan(Plan WorkingPlan) {
		...
		List<ParteBaja> partesBajaPlanActivo = parteBajaRepository.findAllByCiudadanoIdPlanAndDeletedFalse(WorkingPlan);
	...
	public void borrarParteBaja(long idParteBaja) {
		...
		ParteBaja parteBaja = parteBajaRepository.findById(idParteBaja)
				.orElseThrow(() -> new GenericNotFoundException());
		...
		Ciudadano trabajador = ciudadanoRepository.findById(idCiudadano)
