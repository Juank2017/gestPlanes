...
public class EquipoController {
	...
	ResponseEntity<ApiResponse> removeComponent(@PathVariable Long idPlan, @PathVariable Long idEquipo,
		...
		response.getPayload().add(equipoService.equipo(idPlan, idEquipo));
		response.setMensaje("Trabajador " + ciudadano.getNombre() + " " + ciudadano.getApellido1()
				+ " eliminado del equipo " + equipo.getNombreEquipo());

		return ResponseEntity.ok(response);
	}

	@PutMapping("/addComponent/{idPlan}/{idEquipo}/{nuevoComponente}")
	ResponseEntity<ApiResponse> addComponent(@PathVariable Long idPlan, @PathVariable Long idEquipo,
			@PathVariable String nuevoComponente) {
		ApiResponse response = new ApiResponse();
		...
		if (ciudadano.isEmpty()) {
			...
			if (ciudadano.size() > 1) {
				...
				response.getPayload().add(equipoService.equipo(idPlan, idEquipo));
				response.setMensaje("Trabajador " + ciudadano.get(0).getNombre() + " " + ciudadano.get(0).getApellido1()
						+ " añadido al equipo " + equipo.getNombreEquipo());
			}
		}
		;
	...
	@PostMapping("/editaEquipo")
	ResponseEntity<ApiResponse> editaEquipo(@RequestBody EditaEquipoDTO equipo) {

		ApiResponse response = new ApiResponse();

		response.setEstado(HttpStatus.OK);
		Equipo equipoActualizado = equipoService.editarEquipo(equipo);
